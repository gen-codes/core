...
function inlinePartials(partial, generator) {
  const partials = getRegexResults(/\{\{\> (?<name>[a-z0-9]*) (?<args>.*?)\}\}/gmi, partial);
  // partial.match(/\{\{\>(.*?) (.*?)\}\}/gmi)
  for(const partial of partials) {
    const args = partial.args.split(" ");
    let inlinePartial = generator.partials[partial.name];
    for(const arg of args) {
      inlinePartial = `{{with ${arg}}}${inlinePartial}{{/with}}`;
    }
    partial = partial.replace(partial.fullText, inlinePartial);
  }
  return partial;
}
function inlineDynamicPartials(component, generator) {
  ...
  for(const partial of partials) {
    const args = partial.args.split(" ");
    let inlinePartial = Object.keys(generator.partials)
...
export function runGenerator(generator, context, fileTree, output="./"){
  // for(const partialName in generator.partials) {
  //   let partial = generator.partials[partialName]
  //   // partial = inlinePartials(partial, generator);
  //   // partial = inlineDynamicPartials(partial, generator);
  //   generator.partials[partialName] = partial
  // }
  ...
  for(let partialName in generator.partials) {
    hb.registerPartial(
      partialName,
      generator.partials[partialName]
    )
  }
  let reverseCase = {}
  const memoize = fn => { //1
  ...
  const flattenedFiles = flattenFileTree(files)
    for(const fileName in flattenedFiles) {
      fs.outputFileSync(`${output}${fileName}`, flattenedFiles[fileName])
    }
