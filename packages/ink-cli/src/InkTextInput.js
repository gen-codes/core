...
class TextInput extends PureComponent {
	...
	state = {
		cursorOffset:this.props.cursorOffset || (this.props.value || '').length,
		cursorWidth: 0
	}
    ...
    let {cursorOffset, cursorWidth} = this.state;
	...
	handleInput = data => {
		const {value: originalValue, focus, showCursor, mask, onChange, onSubmit, cursorOffset: originalCursorOffset} = this.props;
    ...
    if(cursorWidth!==this.state.cursorWidth)
		this.setState({cursorWidth});
